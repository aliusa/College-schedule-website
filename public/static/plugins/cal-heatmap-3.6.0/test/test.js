...
test("itemSelector accept a valid document.querySelector or CSS3 string value", function () {
    ...
    equal(cal.init({
        itemSelector: "#test > div:last-child",
        paintOnLoad: false
    }), true, "#test > div:last-child is a valid itemSelector");
...
test("itemSelector accept a valid Element object", function () {
    ...
    equal(cal.init({
        ...
        paintOnLoad: false
    }), true, "document.querySelector(\"#a\") is a valid itemSelector");
    ...
    equal(cal.init({
        ...
        paintOnLoad: false
    }), true, "document.getElementById(\"last\") is a valid itemSelector");
    equal($("#last .graph").length, 1, "Graph is appended to #last");

    equal(cal.init({
        itemSelector: document.getElementsByClassName("u")[0],
        paintOnLoad: false
    }), true, "document.getElementsByClassName(\".u\") is a valid itemSelector");
    equal($(".u .graph").length, 1, "Graph is appended to .u");

    equal(cal.init({
        itemSelector: d3.select("[data=y]")[0][0],
        paintOnLoad: false
    }), true, "d3.select(\"[data=y]\")[0][0] is a valid itemSelector");
...
test("Invalid data (undefined) is ignore and treated as an empty object", function () {
    ...
    ok(cal.getDatas(
        ...
        new Date(),
        function () {
            ok(true, true, "Callback argument is called");
        },
        function (data) {
            deepEqual(data, {}, "undefined is equivalent to an empty object");
        }
    ));
});
...
test("Invalid data (null) is ignore and treated as an empty object", function () {
    ...
    ok(cal.getDatas(
        ...
        function (data) {
            deepEqual(data, {}, "null is equivalent to an empty object");
        }
    ));
});
...
test("Invalid data (number) is ignore and treated as an empty object", function () {
    ...
    ok(cal.getDatas(
        ...
        function (data) {
            deepEqual(data, {}, "number is equivalent to an empty object");
        }
    ));
});
...
test("Empty string is treated as an empty object", function () {
    ...
    ok(cal.getDatas(
        ...
        function (data) {
            deepEqual(data, {}, "empty string is equivalent to an empty object");
        }
    ));
});
...
test("Passing directly an object", function () {
    ...
    equal(cal.getDatas(
        ...
        function (data) {
            deepEqual(data, dt, "The passed object is used directly");
        }
    ), false);
});
...
test("OnClick", function () {
    ...
    var testFunction = function (date, itemNb) {
...
test("afterLoadPreviousDomain", function () {
    ...
    var testFunction = function (start, end) {
...
function testColumnsAndRows(domain, subDomain, col, row, expectedCol, expectedRow, skipped) {
    ...
    function _t() {
        ...
        var cal = createCalendar({
            domain: domain, subDomain: subDomain, colLimit: col, rowLimit: row,
...
test("Custom date formatting with custom function", function () {
    ...
    createCalendar({
        start: date, loadOnInit: true, paintOnLoad: true, subDomainDateFormat: function (date) {
            return date.getTime();
        }
    });
