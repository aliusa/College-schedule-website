...
<div class="page_title"><span>{{ classroom.Name }}{% if faculty is not empty %} ({{ faculty }}){% endif %}</span></div>
<br/>
<div>
    <a class="btn btn-warning" id="button_left"
       href="{{ urlFor('classroom', {'id':classroom.ClassroomId, 'date':lastWeekToday}) }}"
       role="button"
       style="float: left">Praėjusi sav.</a>
    <a class="btn btn-warning" id="button_right"
       href="{{ urlFor('classroom', {'id':classroom.ClassroomId, 'date':nextWeekToday}) }}"
       role="button"
    ...
    <div class="content_center">
        ...
        <form name="tstest" method="get" style="display: inline;">
            ...
            <input type="hidden" value="{{ urlFor('classroom', {'id': classroom.ClassroomId, 'date': ''} ) }}" name="linkToSelected"/>
            ...
            {% else %}
                {% for lecture in schedule_item.group %}
                    <tr>
                        ...
                        <td class="content_center sidepanel_right">{{ lecture.ClassroomName }}</td>
                    </tr>
                {% endfor %}
            {% endif %}
        {% endfor %}
